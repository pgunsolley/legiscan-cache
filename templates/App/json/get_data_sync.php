...
$op = $this->getRequest()->getQuery()['op'];

// Data is formatted/mapped explicitely with array literals for clarity
echo json_encode(match ($op) {
    'getSessionList' => [
        'status' => 'OK',
    ...
    'getMasterList' => [
        ...
        'masterlist' => array_map(fn(MasterListRecord $masterListRecord) => [
            ...
            'status_date' => $masterListRecord->status_date,
            'status' => $masterListRecord->status,
            'last_action_date' => $masterListRecord->last_action_date,
    ...
    'getBill' => [
        'status' => 'OK',
        'bill' => array_map(fn(BillRecord $billRecord) => [
            'bill_id' => $billRecord->bill_id,
            'change_hash' => $billRecord->change_hash,
            'session_id' => $billRecord->session_id,
            'session' => [
                'session_id' => $billRecord->bill_record_session->session_id,
                'state_id' => $billRecord->bill_record_session->state_id,
                'year_start' => $billRecord->bill_record_session->year_start,
                'year_end' => $billRecord->bill_record_session->year_end,
                'prefile' => $billRecord->bill_record_session->prefile,
                'sine_die' => $billRecord->bill_record_session->sine_die,
                'prior' => $billRecord->bill_record_session->prior,
                'special' => $billRecord->bill_record_session->special,
                'session_tag' => $billRecord->bill_record_session->session_tag,
                'session_title' => $billRecord->bill_record_session->session_title,
                'session_name' => $billRecord->bill_record_session->session_name,
            ],
            'url' => $billRecord->url,
            'state_link' => $billRecord->state_link,
            'completed' => $billRecord->completed,
            'status' => $billRecord->status,
            'status_date' => $billRecord->status_date,
            'progress' => array_map(fn(BillRecordProgress $billRecordProgress) => [
                'date' => $billRecordProgress->date,
                'event' => $billRecordProgress->event,
            ], $billRecord->bill_record_progresses),
            'state' => $billRecord->state,
            'state_id' => $billRecord->state_id,
            'bill_number' => $billRecord->bill_number,
            'bill_type' => $billRecord->bill_type,
            'bill_type_id' => $billRecord->bill_type_id,
            'body' => $billRecord->body,
            'body_id' => $billRecord->body_id,
            'current_body' => $billRecord->current_body,
            'current_body_id' => $billRecord->current_body_id,
            'title' => $billRecord->title,
            'description' => $billRecord->description,
            'pending_committee_id' => $billRecord->pending_committee_id,
            ...
            ], $billRecord->bill_record_committees),
            ...
            ], $billRecord->bill_record_referrals),
            ...
            ], $billRecord->bill_record_histories),
            ...
            ], $billRecord->bill_record_sponsors),
            ...
            ], $billRecord->bill_record_subjects),
            ...
            'amendments' => array_map(fn(BillRecordAmendment $billRecordAmendment) => [
                ...
                'date' => $billRecordAmendment->date, // TODO: Format to match legiscan output
            ...
            ], $billRecord->bill_record_amendments),
            ...
            ], $billRecord->bill_record_supplements),
            ...
            ], $billRecord->bill_record_calendars),
        ], $data),
    ],
});
